#!/usr/bin/env node
/** Inspect the public Grok provider composition without invoking the product. */
...
if (configPath === undefined || bundlePatchPath === undefined) {
  throw new Error('subagent-grok Loader composition driver requires config and Bundle patch paths')
}

let starts = 0
const ctx = await bootProductionProfile({
  binName: 'subagent-grok-loader-composition',
  profile: 'headless',
  overlayPaths: [
...
try {
  const providerNames = ['grok', 'grok-primary', 'grok-secondary'] as const
  const toolNames = [
    'subagent_grok',
  ...
  const providers = providerNames.map((providerName) => {
    ...
    if (provider === undefined) throw new Error(`${providerName} provider was not registered`)
    return {
      name: provider.name,
