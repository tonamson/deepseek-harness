...
describe('dsh-tool-orc', () => {
  ...
  it('puts Superpowers requirements in DeepSeek prompts and the native envelope in Codex prompts', async () => {
    ...
    expect(review).toContain('"location":"<non-empty>"')
    expect(review).toContain('"evidence":"<non-empty>"')
    expect(review).toContain('"remediation":"<non-empty>"')
    expect(review).toContain('"status":"open"')
