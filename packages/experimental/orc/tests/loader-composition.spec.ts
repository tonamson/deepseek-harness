...
it('replays the authored ORC corpus through durable completion before its final tool result', async () => {
  const text = await readFile(new URL('../../../../snapshots/session/orc-superpowers/session.v4.jsonl', import.meta.url), 'utf8')
  const rows = text.trim().split('\n').map(line => JSON.parse(line) as SessionEvent)
  const orcEvents = rows.filter(row => row.type.startsWith('orc/')).map(row => ({ type: row.type, data: row.data } as OrcEvent))
  ...
  const result = next.data.message
  expect(result).toMatchObject({ role: 'tool', toolCallId: 'call-final-gates', isError: false })
