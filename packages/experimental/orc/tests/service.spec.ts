...
async function setup(config: OrcServiceConfig = CONFIG): Promise<Harness> {
  ...
  return { ctx, service: ctx.orc, supervisor: asAgent(ctx, 'supervisor'), fake: fakeSubagents(ctx), fiber }
}
...
describe('ORC delegated results', () => {
  ...
  it('closes an open Codex row when phase append or flush fails and does not resume it', async () => {
    ...
    const persistence = flushed.ctx.get('sessionPersistence')
    if (!(persistence instanceof FakePersistence)) throw new Error('FakePersistence was not mounted')
