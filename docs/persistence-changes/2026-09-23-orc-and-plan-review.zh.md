...
[English](2026-09-23-orc-and-plan-review.md) | 中文

## 概述

为 ORC 工作流增加 plan/review 与 orc/* 会话事件。
...
这些是会话格式版本 3 的新根。已有日志不含这些事件，仍然有效。不认识某个类型的读取方会拒绝带有该事件的日志，因为这些事件在读取时是必需的。版本 1 的 orc/spec/requested 与 orc/plan/requested 要求 contextRef；这些事件体没有更早的可选形式。SESSION_FORMAT_VERSION 仍为 3。

<a id="verification"></a>
## 验证

pnpm exec vitest run packages/experimental/orc packages/experimental/tool-orc packages/experimental/orc-profile：98 个测试通过。DSH_EXAMPLE_MODE=lib 对 snapshots/session/orc-superpowers 的回放通过两次，覆盖 spec、plan、plan/review 批准、Lead、Peer、review、audit 与修复迭代。
